...
def main():
    ...
    for op_cat, op_names in op_category_to_op_name_map.items():
        ...
        if op_cat == 'GEMM':
            ...
            df_ops = perf_analyzer.build_df_perf_metrics(op_events_uids, bwd=False, non_data_mov=True)
            ...
            df_ops_fwd = perf_analyzer.build_df_perf_metrics(op_events_uids, bwd=False, non_data_mov=True)
            df_ops_fwd = perf_analyzer.summarize_df_perf_metrics(df_ops_fwd, ['mean'])
            df_ops_bwd = perf_analyzer.build_df_perf_metrics(op_events_uids, bwd=True, non_data_mov=True)
