...
ReduceConfig setReduceConfig(const TensorIterator& iter){
  ...
  // Control the number of threadblocks by adjusting the maximum number of threads per multi-processor.
  const int max_threads_per_mp = at::cuda::getCurrentDeviceProperties()->maxThreadsPerMultiProcessor;
#ifdef USE_ROCM
  if (iter.ndim() == 1)
...
#ifdef USE_ROCM
    // In cases where a number of threadblocks along the y direction of the grid is needed then
    // make sure they are reduced to the number of MPs. For smaller sizes, use half the number of
    // MPs. For smaller sizes than half the number of MPs use the original value.
