  ...
  for (int64_t z = blockIdx.z; z < outer_dim; z += gridDim.z) {
    ...
    if (idx < numel) {
      ...
      if (idx == 0 || sorted_indices[idx] != sorted_indices[idx - 1]) {
        ...
        // if (num_duplicates > 63) printf("DUPS = %d\n", num_duplicates);
        ...
        //       for (int dups = 0; dups < num_duplicates; dups++) {
        //         const int64_t grad_row = ((int64_t) indices[idx + dups]) * stride + z * numel * stride;
        //         gradient = static_cast<opmath_t>(grad_output[grad_row + feature_dim]);
        //         grad_weight[weight_row + feature_dim] += static_cast<scalar_t>(gradient);
        //       }
        //       feature_dim += gridDim.y * blockDim.x;
        //     }
        //   }
        ...
        if (num_duplicates < C10_WARP_SIZE) {
          for (int j = 0; j < blockDim.x; j++) {
            int64_t feature_dim = blockIdx.y * blockDim.x + j;
            while (feature_dim < stride) {
              scalar_t gradient = grad_weight[weight_row + feature_dim];
              for (int dups = 0; dups < num_duplicates; dups++) {
                const int64_t grad_row = ((int64_t) indices[idx + dups]) * stride + z * numel * stride;
                gradient += grad_output[grad_row + feature_dim];
              }
              grad_weight[weight_row + feature_dim] = gradient;
              feature_dim += gridDim.y * blockDim.x;
            }
          }
        } else {
    ...
    for (int subwarp = 0; subwarp < C10_WARP_SIZE; subwarp++) {
      ...
      for (int j = 0; j < blockDim.x; j++) {
        ...
        while (feature_dim < stride) {
          ...
          // opmath_t gradient = (opmath_t)0.0;
          // // if (laneIdx == 0) {
          // gradient = static_cast<opmath_t>(grad_weight[new_weight_row + feature_dim]);
          // // }
          ...
          for (int offset = C10_WARP_SIZE / 2; offset > 0; offset /= 2) {
            // gradient += static_cast<scalar_t>(WARP_SHFL_DOWN(static_cast<opmath_t>(gradient), offset));
            gradient += WARP_SHFL_DOWN(gradient, offset);
          }

          if (laneIdx == 0) {
            for (int64_t i = num_warp_passes * C10_WARP_SIZE; i < new_num_duplicates; ++i) {
            // for (int64_t i = 0; i < new_num_duplicates; ++i) {
              grad_row = ((int64_t) indices[new_idx + i]) * stride + z * numel * stride;
              gradient += static_cast<opmath_t>(grad_output[grad_row + feature_dim]);
            }

            grad_weight[new_weight_row + feature_dim] += static_cast<scalar_t>(gradient);
          }
...
__global__ void indexing_backward_kernel_rocm(
  ...
  using opmath_t = at::opmath_type<scalar_t>;
  for (int64_t z = blockIdx.z; z < outer_dim; z += gridDim.z){
    int64_t idx = (blockIdx.x * blockDim.y + threadIdx.y) * C10_WARP_SIZE + threadIdx.x;
    if (idx < numel) {
      int64_t crnt_sorted_idx = sorted_indices[idx];
...
#undef KERNEL_SMEM
        ...
        if (sliceSize <= warp_size) {
          ...
          dim3 new_grid(ceil_div(num_indices, (int64_t) (indices_per_block * warp_size)), grid.y, grid.z);
          size_t smem_dups_size = indices_per_block * warp_size * sizeof(int64_t);
          printf("Calling indexing_backward_kernel_rocm grid = (%d, %d, %d), block = (%d, %d, %d) accumulate = %d num_indices = %d stride = %d\n", new_grid.x, new_grid.y, new_grid.z, block.x, block.y, block.z, accumulate, num_indices, sliceSize);
          if (accumulate) {
            AT_DISPATCH_V2(
              expandedValue.scalar_type(),
              "indexing_backward",
            ...
            AT_DISPATCH_V2(
              ...
              "indexing_backward",
              AT_WRAP([&] {
                indexing_backward_kernel_rocm<scalar_t, false><<<new_grid, block, smem_dups_size, stream>>>(
                  sorted_indices.const_data_ptr<int64_t>(),
                  orig_indices.const_data_ptr<int64_t>(),
                  expandedValue.const_data_ptr<scalar_t>(),
                  src_.mutable_data_ptr<scalar_t>(),
