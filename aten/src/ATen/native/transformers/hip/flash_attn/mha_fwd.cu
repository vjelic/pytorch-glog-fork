        ...
        c10::optional<at::Generator> gen_)
{
    ...
    if (head_size_og % 8 != 0) {
        q_padded = at::pad(q, at::pad({0, 8 - head_size_og % 8}));
        k_padded = at::pad(k, at::pad({0, 8 - head_size_og % 8}));
        v_padded = at::pad(v, at::pad({0, 8 - head_size_og % 8}));
    }
    else {
        q_padded = q;
        k_padded = k;
    ...
    if (return_dropout_randval) {
        ...
        p = at::empty({batch_size, num_heads, seqlen_q, seqlen_k}, opts.dtype(uint8_t));
    }

    uint64_t drop_seed = 1, drop_offset = 0;
    int64_t counter_offset = batch_size * num_heads * ck_tile::get_warp_size();
    auto options = at::TensorOptions().dtype(at::kFloat).device(at::kCUDA);
    auto rng_state = at::empty({2}, options.dtype(int64_t));
