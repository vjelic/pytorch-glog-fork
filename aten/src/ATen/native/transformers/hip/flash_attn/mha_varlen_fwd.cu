               ...
               c10::optional<at::Generator> gen_)
{
    ...
    TORCH_CHECK(cu_seqlens_q.dtype() == at::kInt32, "cu_seqlens_q must have dtype int32");
    TORCH_CHECK(cu_seqlens_k.dtype() == at::kInt32, "cu_seqlens_k must have dtype int32");
    ...
    if (head_size_og % 8 != 0) {
        q_padded = at::pad(q, at::PadFuncOptions({0, 8 - head_size_og % 8}));
        k_padded = at::pad(k, at::PadFuncOptions({0, 8 - head_size_og % 8}));
        v_padded = at::pad(v, at::PadFuncOptions({0, 8 - head_size_og % 8}));
    }
    else {
        q_padded = q;
        k_padded = k;
    ...
    if (return_dropout_randval) {
        ...
        p = at::empty({num_heads, total_q, max_seqlen_k}, opts.dtype(at::kUInt8));
    }
    ...
    auto rng_state = at::empty({2}, options.dtype(at::kInt64));
